...
namespace simd {
	...
	// ==========================================================
	//  Especializaciones SIMD — DOUBLE2/DOUBLE4 (x86/x64)
	// ==========================================================
	...
	// ==========================================================
	//  Especializaciones SIMD — INT32/UINT32 (x86/x64/neon)
	// ==========================================================
#if defined(__ARM_NEON) || defined(__ARM_NEON__)
	...
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			mul<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) {
		// NEON no tiene mullo_epi32 directa; usa widening y narrowing si quisieras 64-bit; aquí vmulq_s32 es válido en AArch64.
#if defined(__aarch64__)
		return simd_pack_t<4, int>(vmulq_s32(a.m, b.m));
#else
		// Fallback escalar por lane si no hay vmulq_s32
		simd_pack_t<4, int> r;
		r.x = a.x * b.x; r.y = a.y * b.y; r.z = a.z * b.z; r.w = a.w * b.w;
		return r;
#endif
	}
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>		mul<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) {
#if defined(__aarch64__)
		return simd_pack_t<4, unsigned>(vmulq_u32(a.m, b.m));
#else
		simd_pack_t<4, unsigned> r;
		r.x = a.x * b.x; r.y = a.y * b.y; r.z = a.z * b.z; r.w = a.w * b.w;
		return r;
#endif
	}
...
#else
	// Fallback escalar por lane si no hay SSE4.1
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			mul<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) { simd_pack_t<4, int> r; r.x=a.x*b.x; r.y=a.y*b.y; r.z=a.z*b.z; r.w=a.w*b.w; return r; }
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>	mul<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) { simd_pack_t<4, unsigned> r; r.x=a.x*b.x; r.y=a.y*b.y; r.z=a.z*b.z; r.w=a.w*b.w; return r; }
#endif

	// min/max requieren SSE4.1 en 32-bit
#if defined(__SSE4_1__)
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			min<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) { return simd_pack_t<4, int>(_mm_min_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>	min<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) { return simd_pack_t<4, unsigned>(_mm_min_epu32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			max<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) { return simd_pack_t<4, int>(_mm_max_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>	max<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) { return simd_pack_t<4, unsigned>(_mm_max_epu32(a.m, b.m)); }
#else
	// Fallback con compare+blend si no hay SSE4.1
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			min<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) {
	...
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>		min<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) {
	...
	template<> SIMD_FORCEINLINE simd_pack_t<4, int>			max<4, int>(const simd_pack_t<4, int>& a, const simd_pack_t<4, int>& b) {
		__m128i mask = _mm_cmpgt_epi32(a.m, b.m);
		return simd_pack_t<4, int>(_mm_or_si128(_mm_and_si128(mask, a.m), _mm_andnot_si128(mask, b.m)));
	}
	template<> SIMD_FORCEINLINE simd_pack_t<4, unsigned>		max<4, unsigned>(const simd_pack_t<4, unsigned>& a, const simd_pack_t<4, unsigned>& b) {
		__m128i as = _mm_sub_epi32(a.m, _mm_set1_epi32(0x80000000u));
		__m128i bs = _mm_sub_epi32(b.m, _mm_set1_epi32(0x80000000u));
	...
	template<> SIMD_FORCEINLINE simd_pack_t<8, int>			add<8, int>(const simd_pack_t<8, int>& a, const simd_pack_t<8, int>& b) { return simd_pack_t<8, int>(_mm256_add_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, unsigned>	add<8, unsigned>(const simd_pack_t<8, unsigned>& a, const simd_pack_t<8, unsigned>& b) { return simd_pack_t<8, unsigned>(_mm256_add_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, int>			sub<8, int>(const simd_pack_t<8, int>& a, const simd_pack_t<8, int>& b) { return simd_pack_t<8, int>(_mm256_sub_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, unsigned>	sub<8, unsigned>(const simd_pack_t<8, unsigned>& a, const simd_pack_t<8, unsigned>& b) { return simd_pack_t<8, unsigned>(_mm256_sub_epi32(a.m, b.m)); }

	template<> SIMD_FORCEINLINE simd_pack_t<8, int>			mul<8, int>(const simd_pack_t<8, int>& a, const simd_pack_t<8, int>& b) { return simd_pack_t<8, int>(_mm256_mullo_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, unsigned>	mul<8, unsigned>(const simd_pack_t<8, unsigned>& a, const simd_pack_t<8, unsigned>& b) { return simd_pack_t<8, unsigned>(_mm256_mullo_epi32(a.m, b.m)); }

	template<> SIMD_FORCEINLINE simd_pack_t<8, int>			min<8, int>(const simd_pack_t<8, int>& a, const simd_pack_t<8, int>& b) { return simd_pack_t<8, int>(_mm256_min_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, unsigned>	min<8, unsigned>(const simd_pack_t<8, unsigned>& a, const simd_pack_t<8, unsigned>& b) { return simd_pack_t<8, unsigned>(_mm256_min_epu32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, int>			max<8, int>(const simd_pack_t<8, int>& a, const simd_pack_t<8, int>& b) { return simd_pack_t<8, int>(_mm256_max_epi32(a.m, b.m)); }
	template<> SIMD_FORCEINLINE simd_pack_t<8, unsigned>	max<8, unsigned>(const simd_pack_t<8, unsigned>& a, const simd_pack_t<8, unsigned>& b) { return simd_pack_t<8, unsigned>(_mm256_max_epu32(a.m, b.m)); }
#endif
