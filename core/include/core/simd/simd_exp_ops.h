...
namespace simd
{
	...
	template <>	SIMD_FORCEINLINE float sqrt<float>(const float& v) { return std::sqrt(v); }
	template <>	SIMD_FORCEINLINE double sqrt<double>(const double& v) { return std::sqrt(v); }
}
...
namespace simd {
	namespace helpers {
		...
		template<typename T>	SIMD_FORCEINLINE T _poly_exp2_fast(T f) {
		...
		template<typename T>	SIMD_FORCEINLINE T _poly_exp2_precise(T f) {
		...
		template<typename T>	SIMD_FORCEINLINE T _poly_log2_fast(T m) {
			// m = 1 + y, y∈[0,1)
			T y = m - T(1);
		...
		template<typename T>	SIMD_FORCEINLINE T _poly_log2_precise(T m) {
...
namespace simd
{
	...
	template <> SIMD_FORCEINLINE simd_pack_t<4, float>	sqrt(const simd_pack_t<4, float>& v) { return simd_pack_t<4, float>(vsqrtq_f32(v.m)); }

	template <> SIMD_FORCEINLINE simd_pack_t<4, float>	rsq(const simd_pack_t<4, float>& v) {
	...
	// ============================================================
#if defined(__aarch64__)
	template <> SIMD_FORCEINLINE simd_pack_t<2, float> sqrt(const simd_pack_t<2, float>& v) { return simd_pack_t<2, float>(vsqrt_f32(v.m)); }

	template <> SIMD_FORCEINLINE simd_pack_t<2, float> rsq(const simd_pack_t<2, float>& v) {
...
#elif (defined(__ARM_NEON) || defined(__ARM_NEON__)) && !defined(__aarch64__)
	template <> SIMD_FORCEINLINE simd_pack_t<2, float>	sqrt(const simd_pack_t<2, float>& v) { return simd_pack_t<2, float>(neon_sqrt_via_rsqrt_f32x2(v.m)); }
	template <> SIMD_FORCEINLINE simd_pack_t<2, float>	rsq(const simd_pack_t<2, float>& v) { return simd_pack_t<2, float>(neon_rsqrt_nr_f32x2(v.m)); }
#else
	template <> SIMD_FORCEINLINE simd_pack_t<2, float>	sqrt(const simd_pack_t<2, float>& v) { return simd_pack_t<2, float>(std::sqrt(v.x), std::sqrt(v.y)); }
	template <> SIMD_FORCEINLINE simd_pack_t<2, float>	rsq(const simd_pack_t<2, float>& v) { return simd_pack_t<2, float>(1.f / std::sqrt(v.x), 1.f / std::sqrt(v.y)); }
#endif

#if defined(__aarch64__)
	template <> SIMD_FORCEINLINE simd_pack_t<2, double>	sqrt(const simd_pack_t<2, double>& v) { return simd_pack_t<2, double>(vsqrtq_f64(v.m)); }

	template <> SIMD_FORCEINLINE simd_pack_t<2, double>	rsq(const simd_pack_t<2, double>& v) {
		float64x2_t s = vsqrtq_f64(v.m);
		float64x2_t one = vdupq_n_f64(1.0);
		return simd_pack_t<2, double>(vdivq_f64(one, s));
	}
#elif defined(__SSE2__) || defined(_M_X64)
	template <> SIMD_FORCEINLINE simd_pack_t<2, double>	sqrt(const simd_pack_t<2, double>& v) { return simd_pack_t<2, double>(_mm_sqrt_pd(v.m)); }
...
#else
	template <> SIMD_FORCEINLINE simd_pack_t<2, double>	sqrt(const simd_pack_t<2, double>& v) { return simd_pack_t<2, double>(std::sqrt(v.x), std::sqrt(v.y)); }
	...
	template <> SIMD_FORCEINLINE simd_pack_t<4, double>	rsq(const simd_pack_t<4, double>& v) {
		__m256d s = _mm256_sqrt_pd(v.m);
		__m256d one = _mm256_set1_pd(1.0);
		return simd_pack_t<4, double>(_mm256_div_pd(one, s));
	}
#else
	template <> SIMD_FORCEINLINE simd_pack_t<4, double>	sqrt(const simd_pack_t<4, double>& v) { return simd_pack_t<4, double>(std::sqrt(v.x), std::sqrt(v.y), std::sqrt(v.z), std::sqrt(v.w)); }
	template <> SIMD_FORCEINLINE simd_pack_t<4, double>	rsq(const simd_pack_t<4, double>& v) { return simd_pack_t<4, double>(1.0 / std::sqrt(v.x), 1.0 / std::sqrt(v.y), 1.0 / std::sqrt(v.z), 1.0 / std::sqrt(v.w)); }
#endif
...
namespace simd {
	...
	template<> SIMD_FORCEINLINE float  fast_exp2<float >(const float& x) {
		...
		float f = x - (float)k;
		float a = helpers::_poly_exp2_fast(f);
		return a * helpers::_exp2i_bits(k);
	}
	template<> SIMD_FORCEINLINE double fast_exp2<double>(const double& x) {
		int    k = (int)x; if (x < 0 && double(k) != x) --k;
		double f = x - (double)k;
		double a = helpers::_poly_exp2_fast(f);
		return a * helpers::_exp2i_bits_d(k);
	}
