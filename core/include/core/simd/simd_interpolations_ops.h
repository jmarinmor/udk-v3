...
namespace simd
{
	...
	template<int D, typename T>	SIMD_FORCEINLINE simd_pack_t<D, T> clamp(const simd_pack_t<D, T>& x, T lo, T hi) {return clamp(x, splat<D, T>(lo), splat<D, T>(hi));}
}
...
namespace simd
{
	...
	template<int D, typename T>	SIMD_FORCEINLINE simd_pack_t<D, T>	saturate(const simd_pack_t<D, T>& x) {return clamp(x, T(0), T(1));}
...
namespace simd
{
	...
	template<int D, typename T>	SIMD_FORCEINLINE simd_pack_t<D, T> step(const simd_pack_t<D, T>& edge, const simd_pack_t<D, T>& x) 
	{
		simd_pack_t<D, T> r(T{});
		for (int i = 0;i < D;++i) 
			(&r.x)[i] = ((&x.x)[i] < (&edge.x)[i]) ? T(0) : T(1);
		return r;
	}
	template<int D, typename T> SIMD_FORCEINLINE simd_pack_t<D, T> step(T edge, const simd_pack_t<D, T>& x) {return step(splat<D, T>(edge), x);}
	template<int D, typename T>	SIMD_FORCEINLINE simd_pack_t<D, T> step(const simd_pack_t<D, T>& edge, T x) {return step(edge, splat<D, T>(x));}
